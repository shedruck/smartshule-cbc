...
class Index extends Public_Controller
{
    ...
    function login()
    {
        ...
        { //check to see if the user is logging in
            ...
            if ($this->ion_auth->login($this->input->post('email'), $this->input->post('password'), $remember))
            {
                ...
                $this->acl->cp_event('login', ['page' => 'Frontend', 'group' => ucwords($gp->name)]);
                $this->session->set_flashdata('message', array('type' => 'success', 'text' => $this->ion_auth->messages()));
                redirect('account');
            }
            else
            { //if the login was un-successful
                $this->acl->cp_event('failed_login', ['page' => 'Frontend']);
     ...
     * @author Brian Mosoti
     * */
    public function random_number($maxlength = 6)
    {
    ...
    public function forgotPassword()
    {
        if (isset($_POST['f_btn']))
        {
            ...
            $this->load->model('sms/sms_m');
            if (!$this->ion_auth_model->get_by_email($email))
            {
                echo "<div class='alert alert-danger col-md-6'>$email is not registered with us</div>";
                redirect('index/forgotPassword');
            }
            else
            {

                // echo "<div class='alert alert-info col-md-3'>Success!! An email with instructions on how to reset your account has been sent to you</div>";
                $id = $this->ion_auth_model->get_by_email($email)->id;
                $phone = $this->ion_auth_model->get_by_email($email)->phone;
                //updating the code into the database
                $this->ion_auth->updateforgotten_password_code($id, $code);
                //send the code as a message and redirect the user to a reset password code based on the code provided
                $message = "Your forgot password code is $code";
                $this->sms_m->send_sms($phone, $message);
                redirect('index/forgotPasswordCode');
            }
        }
    ...
    public function forgotPasswordCode()
    {
        if (isset($_POST['code_submit']))
        {
            ...
            if (!$this->ion_auth_model->get_by_code($code))
            {
                echo 'Please enter correct code';
            }
            else
            {
                $id = $this->ion_auth_model->get_by_code($code)->id;
                redirect('index/setNewPassword/' . $id);
            }
        }
        ...
        $this->template
          ->set_layout('login')
          ->title('Admin', 'Forgot Password')
          ->build('admin/forgot_password_code', $this->data);
    }
    ...
    public function setNewPassword($id)
    {
        ...
        {
            $new = $this->input->post('password_1');
            $identity = $id;
            $new_password = $this->input->post('password_2');
            ...
            {
                $ok = $this->ion_auth_model->reset_password($identity, $new);
            }
        ...
        $this->template
          ->set_layout('login')
          ->title('Admin', 'Forgot Password')
          ->build('admin/set_new_password', $this->data);
    }
    ...
    function rl()
    {
        ...
        $this->template
          ->set_layout('login')
          ->title('Super Admin', 'Renew License')
    ...
    function change_db()
    {
        ...
        echo '<pre>Database Config<br>';
        print_r($this->portal_m->change_db());
        echo '</pre><hr><br>';
        
         echo '<pre>Populate Level<br>';
         print_r($this->portal_m->update_exams_mgt_levl());
         echo '</pre>';
         die();
    }
