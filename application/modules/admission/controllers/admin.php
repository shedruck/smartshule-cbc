...
class Admin extends Admin_Controller
{
    ...
    function update_scholar()
    {
        ...
        foreach ($all as $p)
        {
            ...
            $this->admission_m->update_attributes($p->id, array('boarding_day' => 'Day'));
        }
    ...
    function create($page = 0)
    {
        ...
        $data['transport'] = $this->fee_structure_m->get_transport_rts();
        ...
        if ($this->form_validation->run())
        {
            ...
            else if ($this->input->post('ptype') == 1) //new
            {
                ...
                if ($pid)
                {
                    ...
                    $pdata = array(
                        'first_name' => strtoupper($this->input->post('parent_fname')),
                        'last_name' => strtoupper($this->input->post('parent_lname')),
                        'f_middle_name' => strtoupper($this->input->post('f_middle_name')),
                        'm_middle_name' => strtoupper($this->input->post('m_middle_name')),
                        'email' => $pemail,
                        ...
                        'mother_fname' => strtoupper($this->input->post('mother_fname')),
                        'mother_lname' => strtoupper($this->input->post('mother_lname')),
            ...
            $sdata = array(
                ...
                'birth_cert_no' => $this->input->post('birth_cert_no'),
                ...
                'admission_number' => $this->input->post('old_adm_no'),
            ...
            $ec = array(
                'parent_id' => $ps_id,
                'student' => $rec,
                'name' => strtoupper($this->input->post('contact_name')),
                'middle_name' => strtoupper($this->input->post('contact_m_name')),
                'last_name' => strtoupper($this->input->post('contact_l_name')),
            ...
            /***** Invoice vote heads*************///
                        ...
                        if($rec)
                        {
                                ...
                                foreach($fee as $key => $value)
                                {
                                       ...
                                       $rf = $this->fee_structure_m->get_extra($value);
                                       $fee_data =  [
                                              'student' => $rec,
                                              'term' => $term,
                                              'year' => $year,
                                       ...
                                       $has_id = $this->fee_structure_m->is_invoiced($value, $rec, $term, $year);

                                       if ($has_id)
                                       {
                                           $rf = $this->fee_structure_m->get_extra($value);
                                           $dem = ($rf && $rf->cycle == '999') ? 1 : 0;
                                           $this->fee_structure_m->invoice_fee($fee_data, $has_id, $dem);
                                           $dem ? $i++ : $j++;
                                       }
                                       else
                                       {
                                           $fee_data['invoice_no'] = $invoice_no;
                                           // insert new invoice
                                           $rid = $this->fee_structure_m->invoice_fee($fee_data);

                                           $details = implode(' , ', $fee_data);
                        ...
                        if($rec && $trans)
                        {
                                $fee = $this->fee_structure_m->get_extra($trans);
                                ...
                                $res = $this->transport_m->find_route($trans);
                                ...
                                $has_rt = $this->transport_m->has_route($rec, $term, $year);
                                if ($has_rt)
                                {
                                        $put = $this->transport_m->set_route_update($has_rt, $tx);
                                        $j++;
                                }
                                else
                                {       
                                        $xx = $this->transport_m->create($tx);
                                        ...
                                        $log = array(
                                            ...
                                            'item_id' => $xx,
                                            'transaction_type' => $this->router->fetch_method(),
                                            'description' => base_url('admin') . '/' . $this->router->fetch_module() . '/' . $this->router->fetch_method() . '/' . $xx,
                                            'details' => $details,
                                            'created_by' => $user->id,
                                            'created_on' => time()
                                        );
    ...
    {
        $data['students'] = $this->admission_m->male_std();
    }

        public function bulk_editor()
        {
            $data['houses'] = $this->admission_m->populate('house', 'id', 'name');

            $this->template->title(' Bulk Edit Students ')->build('admin/bulk', $data);
        }

        function get_pending_edit()
        {
            $per = $this->input->get('per_page', 10);
            $search = $this->input->get('search', '');
            $tab = $this->input->get('tab', '');
            $page = $this->input->get('page', 0);
            $class = $this->input->get('class', 0);
            $start = ($page - 1) * $per;

            $houses = $this->admission_m->populate('house', 'id', 'name');
            $result = $this->admission_m->get_all_students($start, $per, $tab, $search, $class);

            foreach ($result as $r)
            {
                $mod = date('Y-m-d', $r->modified_on);
                $today = date('Y-m-d');
                $r->class = ($mod == $today) ? 'modd' : '';
                $r->house = ['id' => $r->house, 'name' => isset($houses[$r->house]) ? $houses[$r->house] : '--'];
                $r->sibs = $this->list_siblings($r->parent_id, $r->id);
                $r->show = false;
            }
            $total = $this->admission_m->get_all_students($start, $per, $tab, $search, $class, 1);
        ...
        function assign_parent()
        {
            ...
            if ($post->parent && $post->student)
            {
                $ssn = $this->admission_m->get_assign_parent($post->student);
                if (empty($ssn))
                {
                    //assign parent
                    $fss = [
                ...
                {
                    $this->admission_m->upd_parent($post->student, ['parent_id' => $post->parent, 'modified_on' => time(), 'modified_by' => $this->user->id]);
                }

                echo json_encode(['message' => 'Sibling Added Successfully', 'status' => 200]);
            }
            else
            {
        ...
        function remove_assign_parent()
        {
            $json = file_get_contents('php://input');
            $post = json_decode($json);

            $this->admission_m->remove_assign($post->id);

            echo json_encode(['message' => 'Sibling Removed Successfully', 'status' => 200]);
        }

        function list_siblings($id, $student)
        {
            $asgn = $this->db->select('id, student_id')->where('parent_id', $id)->get('assign_parent')->result();

            $sibs = [];

            foreach ($asgn as $k)
            {
                if ($k->student_id == $student)
                {
                    continue;
                }

                $rw = $this->worker->get_student($k->student_id);
                $sibs[] = ['id' => $k->id, 'student' => $k->student_id, 'name' => $rw->first_name . ' ' . $rw->last_name, 'class' => $rw->cl->name];
            }

            return $sibs;
        }

        function update_student($id, $slot = 0)
        {
            $json = file_get_contents('php://input');
            $post = json_decode($json);
            ...
            if ($slot == 1)
            {
                $this->admission_m->update_attributes($id, ['house' => $post->house, 'modified_on' => time(), 'modified_by' => $this->user->id]);
            }
            else
            {
                $this->admission_m->update_attributes($id, ['first_name' => $post->first_name, 'middle_name' => $post->middle_name, 'last_name' => $post->last_name, 'modified_on' => time(), 'modified_by' => $this->user->id]);
            }

            echo json_encode(['message' => 'Updated Successfully', 'status' => 200]);
        }
