...
parent::__construct();
	...
	public function index()
	{
	   $config = $this->set_paginate_options(); 	//Initialize the pagination class
		...
		$page = ($this->uri->segment(4)) ? $this->uri->segment(4) : 1;
 		...
 		$data['hostels'] = $this->hostels_m->paginate_all($config['per_page'], $page);
	...
	//page number  variable
	 $data['page'] = $page;
            ...
            //load view
            $this->template->title(' Hostels ' )->build('admin/list', $data);
	}
        ...
        $form_data = array(
				...
				'janitor' => $this->input->post('janitor'), 
				'description' => $this->input->post('description'), 
				 'created_by' => $user -> id ,   
				 'created_on' => time()
			);

            $ok=  $this->hostels_m->create($form_data);
		 ...
		 $this->template->title('Add Hostels ' )->build('admin/create', $data);
		}		
	}
	...
	{ 
         ...
         if(!$this->hostels_m-> exists($id) )
             {
             $this->session->set_flashdata('message', array( 'type' => 'warning', 'text' => lang('web_object_not_exist') ) );
            redirect('admin/hostels');
              }
        //search the item to show in edit form
        $get =  $this->hostels_m->find($id); 
        ...
        //find the item to update
            ...
            $done = $this->hostels_m->update_attributes($id, $form_data);
        ...
        if ( $done) 
			...
			else
			{
             ...
             else
             {
               ...
               $data['result'] = $get;
             //load the view and the layout
             $this->template->title('Edit Hostels ' )->build('admin/create', $data);
	}
	...
	function delete($id = NULL, $page = 1)
	{
		...
		//delete the item
	if ( $this->hostels_m->delete($id) == TRUE) 
		{
			  //$details = implode(' , ', $this->input->post());
				$user = $this->ion_auth->get_user();
					$log = array(
...
return $config; 
	...
	{
		$config = array();
		$config['base_url'] = site_url() . 'admin/hostels/index/';
		$config['use_page_numbers'] = TRUE;
	       $config['per_page'] = 100000000;
            $config['total_rows'] = $this->hostels_m->count();
