...
class Exams_m extends MY_Model
{
    ...
    function get_by_group($class)
    {
        return $this->db->where('class', $class)->get('classes')->row();
    }
    ...
    function fetch_done_list($subject, $list_id)
    {
        ...
        foreach ($list as $l)
        {
            ...
            if (count($units))
            {
                return array('subject' => $subject, 'mk' => $l->marks, 'inc' => $l->inc, 'units' => $units);
            }
            else
            {
                return array('subject' => $subject, 'mk' => $l->marks, 'inc' => $l->inc);
            }
        }
    }
   ...
   function get_subject_result_igcse($exam, $student, $subject)
    {
        ...
        if ($srow)
        {
            ...
            if (count($subs))
            {
                ...
                $sheet = array('subject' => $sbk->name, 'sub_id' => $subject, 'marks' => $rw->marks, 'remarks' => $rw->remarks,'effort' => $rw->effort,'created_by' => $rw->created_by, 'opt' => $ttp, 'units' => $fnsubs, 'inc' => $rw->inc);
            }
            else
            {
                $sheet = array('subject' => $sbk->name, 'sub_id' => $subject, 'marks' => $rw->marks,'remarks' => $rw->remarks,'effort' => $rw->effort,'created_by' => $rw->created_by, 'opt' => $ttp, 'inc' => $rw->inc);
            }
    ...
    function get_subject_result($exam, $student, $subject)
    {
        ...
        if ($srow)
        {
            ...
            if (count($subs))
            {
                ...
                $sheet = array('subject' => $sbk->name, 'sub_id' => $subject, 'marks' => $rw->marks,'remarks' => $rw->remarks,  'effort' => $m->effort, 'opt' => $ttp, 'units' => $fnsubs, 'inc' => $rw->inc);
            }
            else
            {
                $sheet = array('subject' => $sbk->name, 'sub_id' => $subject, 'marks' => $rw->marks,'remarks' => $rw->remarks,  'effort' => $m->effort, 'opt' => $ttp, 'inc' => $rw->inc);
            }
     ...
     * @param int $rank
     */
    function get_report($exam, $student, $class, $rank = 1)
    {
        $fn = array();
        $rows = $this->db->select('a.id as id,a.remarks as remarks, a.total as total')
                          ->where('exam_type', $exam)
        ...
        if ($srow)
        {
            ...
            foreach ($marks as $m)
            {
                ...
                if (count($subs))
                {
                    $sheet[] = array('id' => $m->id, 'subject' => $m->subject, 'remarks' => $m->remarks, 'effort' => $m->effort,'marks' => $m->marks, 'priority' => $sbk->priority, 'inc' => $m->inc, 'opt' => $ttp, 'units' => $subs, 'grading' => isset($gdd->grading) ? $gdd->grading : '');
                }
                else
                {
                    $sheet[] = array('id' => $m->id, 'subject' => $m->subject, 'marks' => $m->marks, 'effort' => $m->effort,'remarks' => $m->remarks, 'priority' => $sbk->priority, 'inc' => $m->inc, 'opt' => $ttp, 'grading' => isset($gdd->grading) ? $gdd->grading : '');
                }
            }
            /**
            ...
            foreach ($sheet as $h)
            {
                ...
                //*** Check if current user is not Parent or Student ****//						
                if (!$this->ion_auth->is_in_group($this->user->id, 8) && !$this->ion_auth->is_in_group($this->user->id, 6))
                {

                    if (($h->opt == 2 && $h->inc == 1) || ($h->opt == 0))
                    {
                        $tot += $h->marks;
                        $cat_groups[$cats[$h->subject]][] = array('subject' => $h->subject, 'marks' => $h->marks);
                    }
                }
            }
            /**
            ...
            $mean = count($sheet) ? round($tot / count($sheet), 2) : 0;

            if ($rank > 1)
            {
                $ranked = $this->worker->rank_marks($cat_groups, $rank);

                $fn['ranked'] = $ranked->optimal;
                $fn['dropped'] = $ranked->dropped;
                $fn['total_ranked'] = $ranked->total;
                $mean = round($ranked->total / 7, 2);
            }
     ...
     * @param int $rank
     */
    function get_report_tr_portal($exam, $student, $class = 0, $rank = 1)
    {
        $fn = array();
        $rows = $this->db->select('a.id as id,a.remarks as remarks, a.total as total')
                          ->where('exam_type', $exam)
        ...
        if ($srow)
        {
            ...
            foreach ($marks as $m)
            {
                ...
                if (count($subs))
                {
                    $sheet[] = array('id' => $m->id, 'subject' => $m->subject, 'marks' => $m->marks, 'priority' => $sbk->priority, 'inc' => $m->inc, 'opt' => $ttp, 'units' => $subs, 'grading' => isset($gdd->grading) ? $gdd->grading : '');
                }
                else
                {
                    $sheet[] = array('id' => $m->id, 'subject' => $m->subject, 'marks' => $m->marks, 'priority' => $sbk->priority, 'inc' => $m->inc, 'opt' => $ttp, 'grading' => isset($gdd->grading) ? $gdd->grading : '');
                }
            }
            /**
             * Automatically Drop Duplicate Marks for the same subject
     ...
     * @return type
     */
    function find_recent($exam)
    {
        return $this->db->select('id,title,year,term')->where('id <', $exam)->order_by('id', 'desc')->limit(3)->get('exams')->result();
    }
     ...
     * @return type
     */
    function get_recent_summary($student, $exam)
    {
        $recent = $this->find_recent($exam);
        $ids = array();
        foreach ($recent as $r)
        {
            $ids[] = $r->id;
        }
        $this->select_all_key('exam_pos');
        return $this->db->where($this->dx('student') . " = '" . $student . "'", NULL, FALSE)
                                            ->where($this->dx('exam') . " in (" . implode(',', $ids) . ")", NULL, FALSE)
                                            ->get('exam_pos')
                                            ->result();
    }
    ...
    {
        $query = $this->db->select('*')->order_by('id')->get('exams');
        $dropdowns = $query->result();
        $options = array();
        foreach ($dropdowns as $d)
        {
            $options[$d->id] = $d->title . " - " . $d->year;
        }
        return $options;
    }
     ...
     * @return type
     */
    function pf_exists($student, $exam)
    {
        $row = $this->db->where($this->dx('student') . " = '" . $student . "'", NULL, FALSE)
                          ->where($this->dx('exam') . " = '" . $exam . "'", NULL, FALSE)
                          ->get('exam_pos')
                          ->row();
    ...
    function get_pos($student)
    {
        $this->select_all_key('kcpe_pos');
        $row = $this->db->where($this->dx('student') . " = '" . $student . "'", NULL, FALSE)
                          ->get('kcpe_pos')
                          ->row();

        return empty($row) ? ' - ' : $row->pos;
    }
