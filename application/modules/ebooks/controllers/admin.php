<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	{	   $config = $this->set_paginate_options(); //Initialize the pagination class
                ...
                $data['ebooks'] = $this->ebooks_m->paginate_all($config['per_page'], $page);
	...
	//page number  variable
	 $data['page'] = $page;
            ...
            //load view
            $this->template->title(' Ebooks ' )->build('admin/list', $data);
	}
		...
		if (!empty($_FILES['file']['name']))
            {
			    $dest = FCPATH . "/uploads/ebooks/" . date('Y');	
       ...
       $user = $this -> ion_auth -> get_user();
        $form_data = array(
                ...
                'file_name' => $file_name, 
				'file_path' =>  "/uploads/ebooks/" . date('Y') , 
				'file_size' => $file_size, 
				'file_type' => $file_type, 				
            ...
            $ok =  $this->ebooks_m->create($form_data);
			...
			redirect('admin/ebooks/');
                ...
                {   
                 ...
                 $data['result'] = $get; 
		 //load the view and the layout
		 $this->template->title('Add Ebooks ' )->build('admin/create', $data);
		}		
	}
	...
	{ 
         ...
         if(!$this->ebooks_m-> exists($id) )
             {
             $this->session->set_flashdata('message', array( 'type' => 'warning', 'text' => lang('web_object_not_exist') ) );
            redirect('admin/ebooks');
              }
			  
	 $done = $this->ebooks_m->update_attributes($id, array('status'=>$status));
	...
	{ 
        ...
        $get =  $this->ebooks_m->find($id); 
            ...
            if ($this->form_validation->run() )  //validation has been passed
             ...
             $data['ebooks_m'] = $this->ebooks_m->find($id);
        ...
        //find the item to update
            ...
            $done = $this->ebooks_m->update_attributes($id, $form_data);
        ...
        if ( $done) 
				...
				{
					if ( is_file(FCPATH.'public/uploads/ebooks/files/'.$index) )
						unlink(FCPATH.'public/uploads/ebooks/files/'.$index);
				}
			...
			else
			{
             ...
             else
             {
               ...
               $data['result'] = $get;
             //load the view and the layout
             $this->template->title('Edit Ebooks ' )->build('admin/create', $data);
	}
	...
	function delete($id = NULL, $page = 1)
	{
		...
		if ( !$this->ebooks_m->exists($id) )
		{
			...
			$ebooks_m = $this->ebooks_m->find($id);

		//Save the files into array to delete after
		array_push($files_to_delete, $ebooks_m->file);
 
		//delete the item
		                     if ( $this->ebooks_m->delete($id) == TRUE) 
		{
...
return $config; 
	...
	{
		$config = array();
		$config['base_url'] = site_url() . 'admin/ebooks/index/';
		$config['use_page_numbers'] = TRUE;
	       $config['per_page'] = 100000000000;
            $config['total_rows'] = $this->ebooks_m->count();
