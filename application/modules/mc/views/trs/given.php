		 ...
		 <div class="btn-group">
				...
				<h5 class="text-center"> MY CLASSES</h5>
					...
					foreach ($classes as $cl)
					{
						...
						<button class="btn btn-default  col-md-12 btn-sm " id="post_<?php echo $cl->id;?>" value="<?php echo base_url('mc/trs/post_mc/'.$post->id.'/'.$cl->id.'/'.$this->session->userdata['session_id'])?>" ><i class='fa fa-caret-right'></i> <?php echo strtoupper($cl->name); ?></button>
						<hr>
						</li>
					  ...
					  <table id="datatable-buttons" class="table table-striped table-bordered">
					   ...
					   <tbody>
						   <tr>
							  ...
							  <td>
							  <span class="label label-success"><?php echo $this->portal_m->count_unique('mc_given','done',1,'created_on',$p->created_on); ?></span>
							  </td>
							  <td>
							  <span class="label label-danger"><?php echo $this->portal_m->count_unique('mc_given','done',0,'created_on',$p->created_on); ?></span>
							  </td>
							  <td>
							   <span class="label label-info"><?php echo $this->portal_m->count_unique('mc_given','remarks',1,'created_on',$p->created_on); ?></span>
							  </td>
							  <td width="">
							  <div class="btn-group pull-right">
						  <?php 
						     $d = $this->portal_m->count_unique('mc_given','done',1,'created_on',$p->created_on); 
							 if($d >0){
						  ?>
								 <a class="btn btn-success btn-sm"  href='<?php echo site_url('mc/trs/mc_remarks/'.$p->created_on.'/'.$post->id.'/'.$this->session->userdata['session_id']);?>'><i class='fa  fa-share'></i> Show All</a>
							 <?php }else{?>
							       <a class="btn btn-danger btn-sm" onClick="return confirm('Are you sure you want to revert this posted quiz? action irreversible')" href='<?php echo site_url('mc/trs/revert/'.$p->created_on.'/'.$post->id.'/'.$this->session->userdata['session_id']);?>'><i class='fa  fa-times'></i> Revert</a>
							 <?php }?>	 
							  </div>
							  </td>
						  </tr>
