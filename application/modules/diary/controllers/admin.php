...
class Admin extends Admin_Controller
{
    ...
    public function extra()
    {
        ...
        //load view
        $this->template->title(' Extras Diary ')->build('admin/extras', $data);
    }
    ...
    function  approve()
    {
        ...
        if(empty($items))
        {
           $this->session->set_flashdata('message', array('type' => 'warning', 'text' => 'No Item Selected'));  
           redirect('admin/diary/');

        }

        if($this->input->post('diary'))
        {
            ...
            {
                $ok = $this->diary_m->update_attributes($id, ['status' => 1, 'modified_on' => time(), 'modified_by' => $this->user->id]);
                $i++;
            }

            if($ok)
            {
                $this->session->set_flashdata('message', array('type' => 'success', 'text' => 'Approved '.$i.' items'));  
                redirect('admin/diary/');
            }
            else
            {
        ...
        if($this->input->post('extra'))
        {
            ...
            foreach($items as $key => $id)
            {
                $ok = $this->diary_m->update_ex($id, ['status' => 1, 'modified_on' => time(), 'modified_by' => $this->user->id]);
                $i++;
            }
