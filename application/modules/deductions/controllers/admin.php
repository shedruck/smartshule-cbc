...
parent::__construct();
	...
	public function index()
	{
	   $config = $this->set_paginate_options(); 	//Initialize the pagination class
		...
		$page = ($this->uri->segment(4)) ? $this->uri->segment(4) : 1;
 		...
 		$data['deductions'] = $this->deductions_m->paginate_all($config['per_page'], $page);
	...
	//page number  variable
	 $data['page'] = $page;
            ...
            //load view
            $this->template->title(' Deductions ' )->build('admin/list', $data);
	}
        ...
        $this->form_validation->set_rules($this->validation());
          ...
          $user = $this -> ion_auth -> get_user();
			...
			for($i=0; $i< $size; ++$i){
			  ...
			  $ok =  $this->deductions_m->create($form_data);
		 ...
		 $this->template->title('Add Deductions ' )->build('admin/create', $data);
		}		
	}
	...
	{ 
         ...
         if(!$this->deductions_m-> exists($id) )
             {
             $this->session->set_flashdata('message', array( 'type' => 'warning', 'text' => lang('web_object_not_exist') ) );
            redirect('admin/deductions');
              }
        //search the item to show in edit form
        $get =  $this->deductions_m->find($id); 
        ...
        //find the item to update
            ...
            $done = $this->deductions_m->update_attributes($id, $form_data);
        ...
        if ( $done) 
			...
			else
			{
             ...
             else
             {
               ...
               $data['result'] = $get;
             //load the view and the layout
             $this->template->title('Edit Deductions ' )->build('admin/edit', $data);
	}
	...
	function delete($id = NULL, $page = 1)
	{
		...
		//delete the item
	if ( $this->deductions_m->delete($id) == TRUE) 
		{
			    //$details = implode(' , ', $this->input->post());
				$user = $this->ion_auth->get_user();
					$log = array(
...
return $config; 
	...
	{
		$config = array();
		$config['base_url'] = site_url() . 'admin/deductions/index/';
		$config['use_page_numbers'] = TRUE;
	       $config['per_page'] = 10;
            $config['total_rows'] = $this->deductions_m->count();
