...
parent::__construct();
	...
	public function index()
	{
	   $config = $this->set_paginate_options(); 	//Initialize the pagination class
		...
		$page = ($this->uri->segment(4)) ? $this->uri->segment(4) : 1;
 		...
 		$data['borrow_book'] = $this->borrow_book_m->paginate_all($config['per_page'], $page);
	...
	//page number  variable
	 $data['page'] = $page;
            ...
            //load view
            $this->template->title(' Borrow Book ' )->build('admin/list', $data);
	}
	...
	function create($page = NULL)
	{
		...
		$limit=$this->borrow_book_m->lib_settings();
		$data['limit']=$limit->limit;
		 ...
		 $user = $this -> ion_auth -> get_user();
				...
				$t_borrowed=$this->borrow_book_m->count_book_per_student($this->input->post('student'));
				
				if($t_borrowed==$limit->limit || $t_borrowed>$limit->limit){
				   $this->session->set_flashdata('message', array( 'type' => 'success', 'text' => '<span style="color:red">Sorry, this student has reached the maximum borrow books</span>' ));
							  redirect('admin/borrow_book/create');
				}
	      ...
	      for($i=0; $i< $size; ++$i){
					...
					if($i==$limit->limit){
						break;
						  ...
						  $this->session->set_flashdata('message', array( 'type' => 'success', 'text' => '<span style="">Maximum borrowing has been reached</span>' ));
							  redirect('admin/borrow_book/create');
					} 
						...
						$remarks = $this->input->post('remarks');
						  ...
						  $t_books=$this->borrow_book_m->total_quantity($book[$i]);
						  $t_borrowed=$this->borrow_book_m->borrowed($book[$i]);
				...
				$ok= $this->borrow_book_m->create($table_list);
			...
			redirect('admin/borrow_book/');
                ...
                {   
                 ...
                 $data['result'] = $get; 
				 $data['books']=$this->borrow_book_m->list_books();
		 //load the view and the layout
		 $this->template->title('Add Borrow Book ' )->build('admin/create', $data);
		}		
	}
	...
	{ 
         ...
         if(!$this->borrow_book_m-> exists($id) )
             {
             $this->session->set_flashdata('message', array( 'type' => 'warning', 'text' => lang('web_object_not_exist') ) );
            redirect('admin/borrow_book');
              }
        //search the item to show in edit form
        $get =  $this->borrow_book_m->find($id); 
        ...
        //find the item to update
            ...
            $done = $this->borrow_book_m->update_attributes($id, $form_data);
			...
			else
			{
             ...
             else
             {
               ...
               $data['result'] = $get;
			     $data['books']=$this->borrow_book_m->populate('books','id','title');
             //load the view and the layout
             $this->template->title('Edit Borrow Book ' )->build('admin/edit', $data);
	}
	...
	function delete($id = NULL, $page = 1)
	{
		...
		//delete the item
     if ( $this->borrow_book_m->delete($id) == TRUE) 
		{
			 //$details = implode(' , ', $this->input->post());
			 $user = $this->ion_auth->get_user();
			 $log = array(
...
return $config; 
	...
	{
		$config = array();
		$config['base_url'] = site_url() . 'admin/borrow_book/index/';
		$config['use_page_numbers'] = TRUE;
	       $config['per_page'] = 10;
            $config['total_rows'] = $this->borrow_book_m->count();
