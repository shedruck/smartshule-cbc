...
class Admin extends Admin_Controller
{
        ...
        public function index()
        {
                ...
                $subordinate = $this->subordinate_m->paginate_all($config['per_page'], $page);
                ...
                $data['contracts'] = $this->subordinate_m->populate('contracts', 'id', 'name');
                $data['departments'] = $this->subordinate_m->populate('departments', 'id', 'name');

                //load view
                $this->template->title(' Supporting Staff ')->build('admin/grid', $data);
        }
        ...
        public function inactive()
        {
                ...
                $this->pagination->initialize($config);
                $page = ($this->uri->segment(4)) ? $this->uri->segment(4) : 1;
                $subordinate = $this->subordinate_m->paginate_inactive($config['per_page'], $page);
                ...
                //load view
                $this->template->title('Inactive  Supporting Staff ')->build('admin/grid', $data);
        }
	...
	{ 
            ...
            if (!$id){
                    ...
                    redirect('admin/subordinate/');
            }
         if(!$this->subordinate_m-> exists($id) )
             {
             $this->session->set_flashdata('message', array( 'type' => 'warning', 'text' => lang('web_object_not_exist') ) );
            redirect('admin/subordinate');
              }
			  
          $done = $this->subordinate_m->update_attributes($id, array('status'=>0));
	...
	{ 
         ...
         if(!$this->subordinate_m-> exists($id) )
          ...
          $done = $this->subordinate_m->update_attributes($id, array('status'=>1));
        ...
        public function list_view()
        {
                ...
                //load view
                $this->template->title('  Supporting Staff ')->build('admin/list', $data);
        }

        function upload_subordinate()
        {
                ...
                while (($fileop = fgetcsv($handler, 1000, ",")) !== false)
                {
                       ...
                       $form_data = array(
                        ...
                        $ok = $this->subordinate_m->create($form_data);
                }
        ...
        function create($page = NULL)
        {
                ...
                {         //Validation OK!
			           ...
			           $this->load->library('files_uploader');
                        ...
                        if ($ok) // the information has therefore been successfully saved in the db
                        {
                        $this->subordinate_m->update_attributes($ok, array('staff_no'=>$ok));
						...
						$user = $this->ion_auth->get_user();
						  ...
						  $this->ion_auth->create_log($log);
                                ...
                                $this->session->set_flashdata('message', array('type' => 'success', 'text' => ' Supporting Staff ' . lang('web_create_success')));

                        }
                        else
                        {
                                $this->session->set_flashdata('error', array('type' => 'error', 'text' => ' Supporting Staff ' . lang('web_create_failed')));
                        }
                ...
                else
                {
                        ...
                        $data['result'] = $get;
                        $data['groups'] = $this->subordinate_m->populate('groups', 'id', 'name');
                        $data['contracts'] = $this->subordinate_m->populate('contracts', 'id', 'name');
                        $data['departments'] = $this->subordinate_m->populate('departments', 'id', 'name');
        ...
        function edit($id = FALSE, $page = 0)
        {
                ...
                $get = $this->subordinate_m->find($id);
                ...
                if ($this->form_validation->run())  //validation has been passed
                {
					 ...
					 $file = $get->passport;   
                        ...
                        $done = $this->subordinate_m->update_attributes($id, $form_data);
                        ...
                        if ($done)
                        {
							  ...
							  $details = implode(' , ', $this->input->post());
                                ...
                                $this->session->set_flashdata('message', array('type' => 'success', 'text' => ' Supporting Staff ' . lang('web_edit_success')));
                                redirect("admin/subordinate/");
                        }
                        else
                        {
                ...
                $data['departments'] = $this->subordinate_m->populate('departments', 'id', 'name');
                //load the view and the layout
                $this->template->title('Edit  Supporting Staff ')->build('admin/create', $data);
        }
        ...
        function profile($id = FALSE)
        {
                ...
                $post = $this->subordinate_m->find($id);
                $data['post'] = $post;

                $this->load->model('record_salaries/record_salaries_m');


                $data['group'] = $this->subordinate_m->populate('groups', 'id', 'name');
                $data['contracts'] = $this->subordinate_m->populate('contracts', 'id', 'name');
                $data['departments'] = $this->subordinate_m->populate('departments', 'id', 'name');
                ...
                $this->template->title('  Supporting Staff Profile')->build('admin/view', $data);
        }

        /**
         * Add To Group   Supporting Staff 
         *
         * @param $id
         * @param $page
         */
        function add_groups($id = FALSE)
        {
                ...
                if (isset($memG))
                {
                        ...
                        $done = $this->subordinate_m->insert_gm($gdata);
        ...
        function delete($id = NULL, $page = 1)
        {
                ...
                //delete the item
                if ($this->subordinate_m->delete($id) == TRUE)
                {
						...
						$this->session->set_flashdata('message', array('type' => 'sucess', 'text' => ' Supporting Staff ' . lang('web_delete_success')));
                }
                else
                {
        ...
        {
                $config = array();
                $config['base_url'] = site_url() . 'admin/subordinate/index/';
                $config['use_page_numbers'] = TRUE;
                $config['per_page'] = 100000000000;
                $config['total_rows'] = $this->subordinate_m->count();
