<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	redirect('admin/login');
			...
			$this->load->model('institution_docs_m');
	}
	...
	{	   $config = $this->set_paginate_options(); //Initialize the pagination class
                ...
                $data['institution_docs'] = $this->institution_docs_m->paginate_all($config['per_page'], $page);
	...
	//page number  variable
	 $data['page'] = $page;
            ...
            //load view
            $this->template->title(' Institution Docs ' )->build('admin/list', $data);
	}
        ...
        $this->form_validation->set_rules($this->validation());
            ...
            $ok =  $this->institution_docs_m->create($form_data);
                     ...
                     //$details = implode(' , ', $this->input->post());
						$user = $this->ion_auth->get_user();
							$log = array(
								...
								'details' => 'Institution Docs Uploaded',   
								'created_by' => $user -> id,   
								'created_on' => time()
							);
			...
			redirect('admin/institution_docs/');
		 ...
		 $this->template->title('Add Institution Docs ' )->build('admin/create', $data);
		}		
	}
	...
	{ 
         ...
         if(!$this->institution_docs_m-> exists($id) )
             {
             $this->session->set_flashdata('message', array( 'type' => 'warning', 'text' => lang('web_object_not_exist') ) );
            redirect('admin/institution_docs');
              }
        //search the item to show in edit form
        $get =  $this->institution_docs_m->find($id); 
			...
			$array_thumbnails = array();
			$array_required = array();
                    $data['institution_docs_m'] = $this->Institution_docs_m->find($id);


			$user = $this -> ion_auth -> get_user();
            // build array for the model
			
             $ownership_doc = $get->ownership_doc;
			...
			$institution_cert = $get->institution_cert;
			...
			$incorporation_doc = $get->incorporation_doc;
			...
			$ministry_approval = $get->ministry_approval;
         ...
         $done = $this->institution_docs_m->update_attributes($id, $form_data);
        ...
        if ( $done) 
			...
			else
			{
             ...
             else
             {
               ...
               $data['result'] = $get;
             //load the view and the layout
             $this->template->title('Edit Institution Docs ' )->build('admin/create', $data);
	}
	...
	function delete($id = NULL, $page = 1)
	{
		...
		if ( !$this->institution_docs_m->exists($id) )
		{
			...
			$institution_docs_m = $this->institution_docs_m->find($id);

		//Save the files into array to delete after
		array_push($files_to_delete, $institution_docs_m->ownership_doc);

		array_push($files_to_delete, $institution_docs_m->institution_cert);

		array_push($files_to_delete, $institution_docs_m->incorporation_doc);
 
		//delete the item
	 if ( $this->institution_docs_m->delete($id) == TRUE) 
		{
			 //$details = implode(' , ', $this->input->post());
				$user = $this->ion_auth->get_user();
					$log = array(
			...
			{
				if ( is_file(FCPATH.'public/uploads/institution_docs/files/'.$index) )
					unlink(FCPATH.'public/uploads/institution_docs/files/'.$index);
			}
...
return $config; 
	...
	{
		$config = array();
		$config['base_url'] = site_url() . 'admin/institution_docs/index/';
		$config['use_page_numbers'] = TRUE;
	       $config['per_page'] = 100000000;
            $config['total_rows'] = $this->institution_docs_m->count();
