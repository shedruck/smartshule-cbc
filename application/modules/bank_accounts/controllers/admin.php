...
parent::__construct();
	...
	//page number  variable
	 $data['page'] = $page;
            ...
            //load view
            $this->template->title(' Bank Accounts ' )->build('admin/list', $data);
	}
        ...
        $form_data = array(
            ...
            $ok = $this->bank_accounts_m->create($form_data);
		 ...
		 $this->template->title('Add Bank Accounts ' )->build('admin/create', $data);
		}		
	}
	...
	{ 
         ...
         if(!$this->bank_accounts_m-> exists($id) )
             {
             $this->session->set_flashdata('message', array( 'type' => 'warning', 'text' => lang('web_object_not_exist') ) );
            redirect('admin/bank_accounts');
              }
        //search the item to show in edit form
        $get =  $this->bank_accounts_m->find($id); 
        ...
        //find the item to update
            ...
            $done = $this->bank_accounts_m->update_attributes($id, $form_data);
        ...
        if ( $done) 
			...
			else
			{
             ...
             else
             {
               ...
               $data['result'] = $get;
             //load the view and the layout
             $this->template->title('Edit Bank Accounts ' )->build('admin/create', $data);
	}
	...
	function delete($id = NULL, $page = 1)
	{
		...
		//delete the item
    if ( $this->bank_accounts_m->delete($id) == TRUE) 
		{
