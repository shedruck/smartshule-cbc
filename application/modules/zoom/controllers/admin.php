<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	{	   $config = $this->set_paginate_options(); //Initialize the pagination class
                ...
                $data['zoom'] = $this->zoom_m->paginate_all($config['per_page'], $page);

            //create pagination links
              $data['links'] = $this->pagination->create_links();

             $data['user_groups']=$this->zoom_m->user_groups();
	...
	//page number  variable
	 $data['page'] = $page;
            ...
            //load view
            $this->template->title(' Zoom ' )->build('admin/list', $data);
	}
        ...
        $this->form_validation->set_rules($this->validation());
		 ...
		 $this->template->title('Add Zoom ' )->build('admin/create', $data);
		}		
	}
        ...
        if ( $done) 
			...
			else
			{
             ...
             else
             {
               ...
               $data['result'] = $get;
             //load the view and the layout
             $this->template->title('Edit Zoom ' )->build('admin/create', $data);
	}
...
return $config; 
	...
	{
		$config = array();
		$config['base_url'] = site_url() . 'admin/zoom/index/';
		$config['use_page_numbers'] = TRUE;
	       $config['per_page'] = 10;
            $config['total_rows'] = $this->zoom_m->count();
