<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	{	   		$config = $this->set_paginate_options(); //Initialize the pagination class
                ...
                $data['newsletters'] = $this->newsletters_m->paginate_all($config['per_page'], $page);
	...
	//page number  variable
	 $data['page'] = $page;
            ...
            //load view
            $this->template->title(' Newsletters Admin' )->build('admin/list', $data);
	}
	...
	function create($page = NULL)
	{
        ...
        $form_data = array(
            ...
            $ok=  $this->newsletters_m->create($form_data);
		 ...
		 $this->template->title('Add Newsletters ' )->build('admin/create', $data);
		}		
	}
	...
	{ 
         ...
         if(!$this->newsletters_m-> exists($id) )
             {
             $this->session->set_flashdata('message', array( 'type' => 'warning', 'text' => lang('web_object_not_exist') ) );
            redirect('admin/newsletters');
              }
			//search the item to show in edit form
			$get =  $this->newsletters_m->find($id); 
            ...
            $data['newsletters_m'] = $this->Newsletters_m->find($id);
        ...
        //find the item to update
            ...
            $done = $this->newsletters_m->update_attributes($id, $form_data);
        ...
        if ( $done) 
			...
			else
			{
             ...
             else
             {
               ...
               $data['result'] = $get;
             //load the view and the layout
             $this->template->title('Edit Newsletters ' )->build('admin/create', $data);
	}
	...
	function delete($id = NULL, $page = 1)
	{
		...
		if ( !$this->newsletters_m->exists($id) )
		{
			...
			$newsletters_m = $this->newsletters_m->find($id);

		//Save the files into array to delete after
		array_push($files_to_delete, $newsletters_m->file);
 
		//delete the item
	if ( $this->newsletters_m->delete($id) == TRUE) 
		{
				$user = $this->ion_auth->get_user();
				$log = array(
					'module' =>  $this->router->fetch_module(), 
...
return $config; 
	...
	{
		$config = array();
		$config['base_url'] = site_url() . 'admin/newsletters/index/';
		$config['use_page_numbers'] = TRUE;
	       $config['per_page'] = 10000000;
            $config['total_rows'] = $this->newsletters_m->count();
	...
	function close($id = NULL, $page = 1)
	{
		...
		//change status
	if ( $this->newsletters_m->close($id) == TRUE) 
		{
				$user = $this->ion_auth->get_user();
				$log = array(
					'module' =>  $this->router->fetch_module(), 
			...
			$this->session->set_flashdata('message', array( 'type' => 'sucess', 'text' => lang('web_edit_success') ));
	...
	function open($id = NULL, $page = 1)
	{
		...
		//change status
	if ( $this->newsletters_m->open($id) == TRUE) 
		{
				$user = $this->ion_auth->get_user();
				$log = array(
					'module' =>  $this->router->fetch_module(), 
