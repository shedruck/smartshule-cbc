...
parent::__construct();
	...
	/* if (!$this->ion_auth->is_in_group($this->user->id, 1) && !$this->ion_auth->is_in_group($this->user->id, 7))
        {
			...
			$this->load->model('return_book_fund_m');
	...
	public function index()
	{
	   $config = $this->set_paginate_options(); 	//Initialize the pagination class
		...
		$page = ($this->uri->segment(4)) ? $this->uri->segment(4) : 1;
 		...
 		$data['return_book_fund'] = $this->return_book_fund_m->get_borrowed();
	...
	//page number  variable
	 $data['page'] = $page;
         ...
         $data['stream_name'] = $this->borrow_book_fund_m->populate('class_stream','id','name');
            ...
            //load view
            $this->template->title(' Return Book Fund ' )->build('admin/index', $data);
	}
	...
	public function listing()
	{
	   $config = $this->set_paginate_options(); 	//Initialize the pagination class
		...
		$page = ($this->uri->segment(4)) ? $this->uri->segment(4) : 1;
 		...
 		$data['return_book_fund'] = $this->return_book_fund_m->paginate_all($config['per_page'], $page);
	...
	//page number  variable
	 $data['page'] = $page;
            ...
            //load view
            $this->template->title(' Return Book Fund ' )->build('admin/list', $data);
	}
	...
	function create($id,$page = NULL)
	{
            ...
            if (!$id){
                    ...
                    redirect('admin/return_book_fund/');
            }
           ...
           $data['b_books']=$this->return_book_fund_m-> student_books($id);
        ...
        $this->form_validation->set_rules($this->validation());
 		...
 		for($i=0; $i < $size ; $i++){
						 ...
						 $form_data = array(
								...
								$ok=  $this->return_book_fund_m->create($form_data);
								//Update book status
					 ...
					 $form_update = array( 
							...
							$done = $this->borrow_book_fund_m->update_attributes($book[$i], $form_update);
		 ...
		 $this->template->title('Add Return Book Fund ' )->build('admin/create', $data);
		}		
	}
	...
	{ 
         ...
         if(!$this->return_book_fund_m-> exists($id) )
             {
             $this->session->set_flashdata('message', array( 'type' => 'warning', 'text' => lang('web_object_not_exist') ) );
            redirect('admin/return_book_fund');
              }
        //search the item to show in edit form
        $get =  $this->return_book_fund_m->find($id); 
		 $data['books']=$this->borrow_book_fund_m->populate('books','id','title');
        ...
        //find the item to update
            ...
            $done = $this->return_book_fund_m->update_attributes($id, $form_data);
        ...
        if ( $done) 
			...
			else
			{
             ...
             else
             {
               ...
               $data['result'] = $get;
             //load the view and the layout
             $this->template->title('Edit Return Book Fund ' )->build('admin/create', $data);
	}
	...
	function delete($id = NULL, $page = 1)
	{
		...
		//delete the item
	 if ( $this->return_book_fund_m->delete($id) == TRUE) 
		{
			   //$details = implode(' , ', $this->input->post());
				$user = $this->ion_auth->get_user();
					$log = array(
...
return $config; 
	...
	{
		$config = array();
		$config['base_url'] = site_url() . 'admin/return_book_fund/index/';
		$config['use_page_numbers'] = TRUE;
	       $config['per_page'] = 100;
            $config['total_rows'] = $this->return_book_fund_m->count();
