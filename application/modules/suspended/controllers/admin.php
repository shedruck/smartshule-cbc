<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	{	   $config = $this->set_paginate_options(); //Initialize the pagination class
                ...
                $data['suspended'] = $this->suspended_m->paginate_all($config['per_page'], $page);
	...
	//page number  variable
	 $data['page'] = $page;
            ...
            //load view
            $this->template->title(' Suspended ' )->build('admin/list', $data);
	}
	
	function update_old_suspensions(){
		
		$all_suspended = $this->suspended_m->get_suspended();
		
		foreach($all_suspended as $sp){
			$form_data = array(
				'date' => $sp->modified_on, 
				'reason' => '', 
				'student' => $sp->id ,   
				'created_by' => $sp->modified_by ,   
				'created_on' => time()
			);
          
            $ok=  $this->suspended_m->create($form_data);
		}
        ...
        function create($id=FALSE,$page = NULL)
        {
          ...
          if(!$this->admission_m-> exists($id) )
		   ...
		   //create control variables
            $data['updType'] = 'create';
            $data['ps'] = $this->admission_m->find($id);
            $form_data_aux  = array();
        ...
        $form_data = array(
		 ...
		 $this->template->title('Add Suspended ' )->build('admin/create', $data);
		}		
	}
	...
	{ 
         ...
         if(!$this->suspended_m-> exists($id) )
             {
             $this->session->set_flashdata('message', array( 'type' => 'warning', 'text' => lang('web_object_not_exist') ) );
            redirect('admin/suspended');
              }
        //search the item to show in edit form
        $get =  $this->suspended_m->find($id); 
        ...
        //find the item to update
            ...
            $done = $this->suspended_m->update_attributes($id, $form_data);
        ...
        if ( $done) 
			...
			else
			{
             ...
             else
             {
               ...
               $data['result'] = $get;
             //load the view and the layout
             $this->template->title('Edit Suspended ' )->build('admin/create', $data);
	}
	...
	function delete($id = NULL, $page = 1)
	{
		...
		//delete the item
	if ( $this->suspended_m->delete($id) == TRUE) 
		{
			//$details = implode(' , ', $this->input->post());
						$user = $this->ion_auth->get_user();
							$log = array(
...
return $config; 
	...
	{
		$config = array();
		$config['base_url'] = site_url() . 'admin/suspended/index/';
		$config['use_page_numbers'] = TRUE;
	       $config['per_page'] = 1000000000;
            $config['total_rows'] = $this->suspended_m->count();
