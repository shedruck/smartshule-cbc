...
class Admin extends Admin_Controller
{
        ...
        public function index()
        {
                ...
                if ($this->input->post()) {
                        ...
                        $res = $this->transport_m->filter_transport($route, $class, $term, $year, $stage, $bus);
                        foreach ($res as $r) {
                ...
                $list = ($this->input->post()) ? $this->transport_m->filter_transport($route, $class, $term, $year, $stage, $bus) : [];
                if ($this->input->post('export')) {
                        if (empty($list)) {
                                return FALSE;
                        }
                ...
                $data['stages'] = $this->transport_m->populate('transport_stages', 'id', 'stage_name');
                $data['buses'] = $this->transport_m->populate('transport_buses', 'id', 'reg_no');
                $data['amounts'] =  $this->transport_m->get_route_amount();
                //load view
                $this->template->title(' Transport')->build('admin/report', $data);
        }
        ...
        function clean_stds()
        {
                ...
                foreach ($extras as $key => $x) {
                        ...
                        foreach ($students as $s) {
                                ...
                                if ($this->transport_m->remove_extras($s->id)) {
                                        $i++;
                                }
                        }
                }
        ...
        function manage_transport()
        {
                ...
                $data['amounts'] =  $this->transport_m->get_route_amount();
                $data['buses'] = $this->transport_m->populate('transport_buses', 'id', 'reg_no');
                $this->template->title('Manage Transport ')->build('admin/manage_transport', $data);
        }
        ...
        function remove_students()
        {
                ...
                if ($this->input->post()) {
                        ...
                        foreach ($item as $key => $value) {
                                $i++;
                                $ok = $this->transport_m->remove_student($value, 'transport');
                        }
                }


                if ($ok) {
                        $this->session->set_flashdata('message', array('type' => 'success', 'text' => 'Successfully Removed ' . $i . ' Students'));
                } else {
                        $this->session->set_flashdata('message', array('type' => 'error', 'text' => 'Something Went Wrong'));
                }
        ...
        public function assign_bus($id)
        {
                ...
                if (!$this->transport_m->route_exists($id)) {
                        $this->session->set_flashdata('message', ['type' => 'warning', 'text' => lang('web_object_not_exist')]);
                        redirect('admin/transport/routes');
                }
                $route = $this->transport_m->find_route($id);

                if ($this->input->post()) {
                        if (!$this->input->post('bus')) {
                                $this->session->set_flashdata('message', ['type' => 'success', 'text' => "Please select Bus"]);
                                redirect("admin/transport/assign_bus/" . $id);
                        }

                        $bus = $this->input->post('bus');
                        $this->transport_m->assign_bus($id, $bus);
                ...
                $this->template->title('Assign Buses to Route')->build('admin/assign', $data);
        }

        function bus_route($id)
        {
                $buses = $this->transport_m->get_bus_route($id);
                $bs = [];
                $bss = $this->transport_m->populate('transport_buses', 'id', 'reg_no');
                foreach ($buses as $b) {
                        $bs[$b->bus] = isset($bss[$b->route]) ? $bss[$b->route] : '';
                }

                echo json_encode($bs);
        }

        function add_students_to_bus()
        {
                $res = [];
                ...
                $data['buses'] = $this->transport_m->populate('transport_buses', 'id', 'reg_no');
                $data['amounts'] =  $this->transport_m->get_route_amount();
                $this->template->title(' Add to Bus')->build('admin/add_to_bus', $data);
        }

        function add_to_bus()
        {
                $out = ['Status' => 201, 'Message' => 'No Data'];
                if($this->input->post())
                {
                        ...
                        foreach($students as $std => $tr)
                        {
                                 ...
                                 $has_bus = $this->transport_m->exists_in_bus($std);
                                //  echo '<pre>';
                                //  print_r($has_bus);
                                //  echo '</pre>';
                                //  die();
                                 ...
                                 if($has_bus)
                                 {
                                     if($this->transport_m->update_rec('transport_assign', $has_bus, $form_data))
                                     {
                                        $u++;
                                     }   
                                 }
                                 elseif(!$has_bus)
                                 {
                                        if($this->transport_m->save_rec('transport_assign', $form_data))
                                        {
                                                $c++;
                                        }
                                 }
                        ...
                        $mess = 'Updated '.$u.' existing records : Added '.$c.' New records';
                        $out = ['Status' => 200, 'Message' => $mess];
                }
