...
class Mysqldump
{
        ...
        $db = '', $user = '', $pass = '', $host = 'localhost', $type = 'mysql', $dumpSettings = array(), $pdoSettings = array()
        )
        {
                        ...
                        switch ($this->dbType)
                        {
                                ...
                                case 'dblib':
                                        $this->dbHandler = new PDO(
                                                     $this->dbType . ":host=" .
                                                     $this->host . ";dbname=" . $this->db, $this->user, $this->pass, $this->pdoSettings
                                        );
        ...
        public function start($filename = '')
        {
                ...
                $this->compressManager->download($this->fileName);
        }
                ...
                else
                {
                        ...
                        foreach ($this->dbHandler->query($this->typeAdapter->show_tables($this->db)) as $row)
                        {
                                if (in_array(current($row), $this->dumpSettings['include-tables'], true))
                                {
                                        array_push($this->tables, current($row));
                                        $elem = array_search(
                                                     current($row), $this->dumpSettings['include-tables']
                                        );
        ...
        private function getTableStructure($tableName)
        {
                ...
                $columnTypes = array();
                $columns = $this->dbHandler->query($this->typeAdapter->show_columns($tableName), PDO::FETCH_ASSOC
                );
        ...
        /**
         * Inspect Table Columns to determine if encrypted
         * Assumes table is encryped if it has atleast one blob column
         * 
         * @param type $table
         */
        function has_blobs($table)
        {
                $i = 0;
                $str = $this->tableColumnTypes[$table];
                foreach ($str as $col => $specs)
                {
                        $ob = (object) $specs;
                        if (isset($ob->is_blob) && $ob->is_blob == 1)
                        {
                                $i++;
                        }
                }
         ...
         * @param string $tableName Name of table which contains rows
         * @param array $row Associative array of column names and values to be quoted
         * @param array $valid array of valid columns for this table
         * @param array $md5 Key for enc purposes
         * @param array $conn Current Database connection id
         *
         * @author  Ben
         * @return string
         */
        function escape_custom($tableName, $row, $valid = array(), $md5 = '', $conn = '')
        {
                $ret = array();
                $columnTypes = $this->tableColumnTypes[$tableName];
                ...
                foreach ($row as $colName => $colValue)
                {
                        if (!in_array($colName, $valid))
                        {
                                continue;
                        }
                        if (is_null($colValue) || empty($colValue))
                        {
                                $ret[] = "''";
                        }
                        elseif ($columnTypes[$colName]['is_numeric'])
                        ...
                        elseif ($this->dumpSettings['hex-blob'] && $columnTypes[$colName]['is_blob'])
                        {
                                ...
                                {
                                        $ret[] = '0x' . $this->secure_bin($colValue, $md5, $conn);
                                        //$ret[] = '0x' . bin2hex($colValue);
                                }
                        }
                        else
        ...
        /**
         * Quote and Escape Field Value depending on Field Type
         * 
         * @param type $table
         * @param type $field
         * @param type $value
         ...
         * @return type
         */
        function escape_custom_field($table, $field, $value, $md5, $conn)
        {
                $ret = '';
                $columnTypes = $this->tableColumnTypes[$table];
                ...
                elseif ($this->dumpSettings['hex-blob'] && $columnTypes[$field]['is_blob'])
                {
                        if (empty($value))
                        {
                                $ret = "''";
                        ...
                        {
                                $ret = '0x' . $this->secure_bin($value, $md5, $conn);
                        }
                }
                else
                {
                        $ret = $this->dbHandler->quote($value);
                }
         ...
         * @return type
         */
        public function secure_bin($data, $md5, $conn)
        {
                $data2 = "'" . $this->escape_str($data, $conn) . "'";
                $res = $this->dbHandler->query("select HEX(AES_ENCRYPT($data2,SHA2('" . $md5 . "',512))) as data")->fetch();
         ...
         * @return type
         */
        function sample_pdo($cn, $comando)
        {
         ...
         * @return type
         */
        function escape_str($str, $conn, $like = FALSE)
        {
                if (is_array($str))
                {
        ...
        function make_types($table)
        {
                $columnTypes = array();
                $columns = $this->dbHandler->query($this->typeAdapter->show_columns($table), PDO::FETCH_ASSOC
                );
                ...
                $this->tableColumnTypes[$table] = $columnTypes;
                return;
        }

        function extract_cols($table)
        {
                $columns = $this->dbHandler->query($this->typeAdapter->show_columns($table), PDO::FETCH_ASSOC);
                $fncol = array();
                foreach ($columns as $key => $col)
        ...
        /**
         * Table rows extractor custom
         *
         * @param string $tableName  Name of table to export
         *
         * @return null
         */
        public function make_sql_string($table, $rows)
        {
                $sl = chr(13) . chr(10);
                $onlyOnce = true;
                ...
                foreach ($rows as $row)
                {
                        $vals = $this->escape_custom($table, $row, $cols);
                        ...
                        elseif ($row->flag == 2)//update normal
                        {
                                $stmt .= "UPDATE `$table` as s, (SELECT id, AES_DECRYPT($row->col,SHA2('MD5',512)) as $row->col ";
                                $stmt .= "FROM `$table` WHERE id= $row->row_id ) AS p ";
                                $stmt .= $sl . "SET s.$row->col = AES_ENCRYPT(p.$row->col + $row->val,SHA2('MD5',512))";
                                $stmt .= " WHERE s.id = p.id ";
                                $stmt .= "AND s.id= $row->row_id ;";
...
class CompressGzip extends CompressManagerFactory
{
        ...
        public function download($filez)
        {
                $file = $filez . ".gz";
                if (file_exists($file))
                {
        ...
        public function create_trigger($row)
        {
                ...
                if (isset($row['SQL Original Statement']))
                {
                        ...
                        $triggerStmtReplaced = str_replace(
                                     "CREATE DEFINER", "/*!50003 CREATE*/ /*!50017 DEFINER", $triggerStmt
                        );
                        $triggerStmtReplaced = str_replace(
                                     " TRIGGER", "*/ /*!50003 TRIGGER", $triggerStmtReplaced
                        );
