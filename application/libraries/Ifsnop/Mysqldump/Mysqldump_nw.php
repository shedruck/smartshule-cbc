...
class Mysqldump
{
    ...
    private function getHeader()
    {
        ...
        $header = "-- SMART SHULE DATABASE BACKUP UTILITY - http://smartshule.com" . PHP_EOL .
                "--" . PHP_EOL .
        ...
        $ci = &get_instance();
        $header .= "-- Backup By:  " .$ci->ion_auth->get_user()->first_name.' '.$ci->ion_auth->get_user()->last_name.
    ...
    private function getViewStructure($viewName)
    {
        ...
        foreach ($this->dbHandler->query($stmt) as $r) {
            if (isset($r['Create View'])) {
                if (!$this->dumpSettings['no-create-info']) {
                    $this->compressManager->write(
                         "-- Table structure for view `$viewName`" . PHP_EOL .   PHP_EOL
                    );
                    if ($this->dumpSettings['add-drop-table']) {
                        $this->compressManager->write(
...
class TypeAdapterMysql extends TypeAdapterFactory
{
    ...
    public function getDatabaseHeader() {
        ...
        return "-- Current Database: `${args[0]}`" . PHP_EOL. "--" . PHP_EOL ;
     }
}
