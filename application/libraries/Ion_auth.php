...
class Ion_auth
{
    ...
    public function forgotten_password($identity)
        {    //changed $email to $identity
                ...
                if (!$user)
                {
                        $this->ci->session->set_flashdata('message', array('type' => 'error', 'text' => 'A User with that Email does not Exist. Enter Your Correct Email.'));
                        redirect('forgot_password', 'refresh');
                }
                $email = $user->email;
                $key = $this->ci->ion_auth_model->forgotten_password($email);
    ...
    public function forgotten_password_complete($code)
    {
        ...
        $profile = $this->ci->ion_auth_model->get_user_by_code($code ); //pass the code to profile
    ...
    public function is_admin()
    {
        ...
        $ugs = $this->ci->ion_auth->get_user()->groups;
        $grps = array();
        foreach ($ugs as $g)
        {
            $grps[] = strtolower($g->name);
        }

        return in_array(strtolower($admin_group), $grps);
    }
    ...
    public function get_user($id = 0)
    {
        if (!$id)
        {
            $usr = $this->ci->ion_auth_model->user()->row();
        }
        else
        {
            $usr = $this->ci->ion_auth_model->user($id)->row();
        }
        $ugp = $this->ci->ion_auth_model->get_users_groups($id);

        if ($ugp->result())
        {
            $usr->groups = $ugp->result();
        }
        else
        {
            $usr->groups = array();
